...
mutable struct Math_pendulum
	...
	phi::Float64
	phi_dot::Float64
    Math_pendulum(l, g, m, k, c, phi, phi_dot) = new(l, g, m, k, c, phi, phi_dot)
end
...
function create_fig(mp::Math_pendulum, hfig=500, bfig=500, size_factor = 1.1)
	...
	p_mat[:,5] = p_mat[:,1]
	box_border_plot = plot(p_mat[1,:],p_mat[2,:],border=:none,aspect_ratio=1,legend=false,color="gray",lw=1,fmt=:pdf)
	plot!(size=(bfig,hfig))
